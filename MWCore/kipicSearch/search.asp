<%
' ----------------------------------------------------------------------------
' Zoom Search Engine 7.1 (3/April/2017)
' ASP search script
...
Dim MaxKeyWordLineLen, DictIDLen, NumKeywords, NumPages
Dim DictReservedLimit, DictReservedPrefixes, DictReservedSuffixes, DictReservedNoSpaces, DictReservedSpaces
...
Dim UseDomainDiversity, MaxDomainDiversity, div_res_count, MaxResultsPerQuery
...
if (NoSearch = 0) then
	...
	Dim num_recs_found
	Dim rec_multiwords, rec_multiworMW_count
	...
	if (Recommended = 1) then
		for rl = 0 to rec_count-1
			...
			if (InStr(rec_word, ",") > 0) then
				...
				rec_multiworMW_count = UBound(rec_multiwords)
	...
	if (MaxResultsPerQuery > 0) then
		if (MaxResultsPerQuery < matches) then
	...
	do while (arrayline < matches AND arrayline < result_limit)
		...
		if (DisplayContext = 1 AND termsmatched > 0 AND IsNoKeywordQuery = False) then
			...
			for j = 0 to (context_keywords - 1) Step 1
				...
				if (UseDomainDiversity = 1 AND arrayline < div_res_count) then					
					origpos = output(div_res_table(3+j, arrayline))
				else
				...
				for cti = 0 to context_word_count
					...
					if (noSpaceForNextChar = False) then
						...
						elseif (word_id < DictReservedSpaces) then
							context_str = context_str & " "
...
Function porterEndsDoubleConsonent(str)
	...
	If Len(str) >= 2 Then
		...
		holMW_ends = Right(str, 2)
		'checking if both the characters are same
		If Mid(holMW_ends, 1, 1) = Mid(holMW_ends, 2, 1) then
			'check for double consonent
			If holMW_ends = "aa" Or holMW_ends = "ee" Or holMW_ends = "ii" Or holMW_ends = "oo" Or holMW_ends = "uu" Then
				porterEndsDoubleConsonent = False
			Else
				'if the second last character is y, and there are atleast three letters in str
				If holMW_ends = "yy" And Len(str) > 2 Then
